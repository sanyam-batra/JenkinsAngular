...
echo response.data.toString()
  ...
  stage('Connection Test Server') {
    sshagent(['SanyamKey']) {
      ...
      sh 'ssh ec2-user@ec2-52-201-186-196.compute-1.amazonaws.com sudo nohup bash dockerexec.sh & '
  ...
  stage('Test Server Testing') {
    ...
    if(response != "HTTP/1.1 200 OK") {
      ...
      sh 'ssh ec2-user@ec2-52-45-120-161.compute-1.amazonaws.com sudo nohup bash dockerexec.sh &'
    }
    }
  }
}
